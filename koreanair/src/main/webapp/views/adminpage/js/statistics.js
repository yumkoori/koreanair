...
function updatePopularRoutesWidget(responseData) {
    ...
    // 상위 6개 노선만 표시 (기존 App Versions처럼)
    const top6Routes = routesData.slice(0, 6);
    console.log('상위 6개 인기노선:', top6Routes);
    
    // 각 노선에 대해 widget_summary 생성
    top6Routes.forEach((routeData, index) => {
        ...
        const routePercent = parseFloat(routeData.route_percent) || 0;
...
function updateRouteStatsWidget(responseData) {
    ...
    // 새로운 제목 추가
    const titleElement = document.createElement('h4');
    titleElement.textContent = '년도별 인기 항공노선 순위';
    ...
    top6Routes.forEach((routeData, index) => {
        ...
        // 퍼센트 확인
        if (routeData.route_percent !== undefined) {
            routePercent = parseFloat(routeData.route_percent) || 0;
        } else if (routeData.routePercent !== undefined) {
            routePercent = parseFloat(routeData.routePercent) || 0;
        } else if (routeData.percent !== undefined) {
            routePercent = parseFloat(routeData.percent) || 0;
        }
