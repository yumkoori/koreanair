...
$(document).ready(function() {
    ...
    function showLoading() {
        console.log("[CLIENT DEBUG] showLoading() called");
    ...
    function hideLoading() {
        console.log("[CLIENT DEBUG] hideLoading() called");
    ...
    $dateSelectorInput.on('apply.daterangepicker', function(ev, picker) {
        ...
        console.log("[CLIENT DEBUG] Datepicker 'apply' event. New date selected:", newlySelectedDate);
        
        selectedDate = newlySelectedDate; 
        $(this).val(selectedDate);  

        if (currentFilterType === 'realtime') {
            console.log("[CLIENT DEBUG] 'realtime' was active, switching to 'all' filter due to new date selection.");
    ...
    $filterButtons.on('click', function() {
        ...
        console.log("[CLIENT DEBUG] Filter button clicked. New filter:", newFilterType, "Current selectedDate (before this click):", selectedDate);
        ...
        if (newFilterType === 'realtime') {
            ...
            console.log("[CLIENT DEBUG] 'realtime' filter selected. Date set to today:", selectedDate);
        } else {
            dateToQuery = selectedDate;
            console.log("[CLIENT DEBUG] Filter '"+newFilterType+"' selected. Using date:", dateToQuery, "(current global selectedDate)");
        }
    ...
    function loadAndDisplaySchedules(dateToQuery, filterToApply) {
        console.log(
            "[CLIENT DEBUG] loadAndDisplaySchedules - Preparing AJAX call with following values:\n" +
            "  - Date to send (dateToQuery): " + dateToQuery + "\n" +
            "  - Filter to send (filterToApply): " + filterToApply + "\n" +
            "  - Current global selectedDate (JS var): " + selectedDate + "\n" +
            "  - Value in dateSelectorInput field: " + $dateSelectorInput.val() + "\n" +
            "  - Current global currentFilterType (JS var): " + currentFilterType
        );
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log("[CLIENT DEBUG] AJAX Success! Data received for date", dateToQuery, "filter", filterToApply, ":", data);
                currentSchedules = [];

                if (Array.isArray(data) && data.length > 0 && data[0] && data[0].error) {
                    console.error("[CLIENT DEBUG] Server error from JSP (in array):", data[0].error);
                    $flightScheduleTableBody.empty().append('<tr><td colspan="7" class="text-center">스케줄 조회 실패: ' + data[0].error + '</td></tr>');
                } else if (data && typeof data === 'object' && data.error && !Array.isArray(data)) { 
                    console.error("[CLIENT DEBUG] Server error from JSP (object):", data.error);
                     $flightScheduleTableBody.empty().append('<tr><td colspan="7" class="text-center">스케줄 조회 실패: ' + data.error + '</td></tr>');
                } else if (Array.isArray(data)) {
                    currentSchedules = data;
                } else {
                    console.error("[CLIENT DEBUG] Invalid data format. Received:", data);
            ...
            error: function(jqXHR, textStatus, errorThrown) {
                console.error("[CLIENT DEBUG] AJAX Error fetching for date " + dateToQuery + " filter " + filterToApply + ":", textStatus, errorThrown, jqXHR.responseText);
            ...
            complete: function() {
                 console.log("[CLIENT DEBUG] AJAX call complete for date:", dateToQuery, "filter:", filterToApply);
                hideLoading();
            }
        });
    }
    ...
    function renderTable() {
        ...
        if (!Array.isArray(currentSchedules) || currentSchedules.length === 0) {
            let noDataMessage = "선택하신 조건에 해당하는 스케줄이 없습니다.";
            if ($flightScheduleTableBody.find('td[colspan="7"]:contains("실패")').length > 0 ||
                $flightScheduleTableBody.find('td[colspan="7"]:contains("형식의 응답입니다")').length > 0 ||
                $flightScheduleTableBody.find('td[colspan="7"]:contains("오류가 발생했습니다")').length > 0 ||
                $flightScheduleTableBody.find('td[colspan="7"]:contains("불러오고 있습니다")').length > 0) {
            } else {
                 $flightScheduleTableBody.append('<tr><td colspan="7" class="text-center">' + noDataMessage + '</td></tr>');
            }
            return;
        }
    ...
    $paginationControls.on('click', 'a.page-link', function(e) {
        ...
        if (!isNaN(page) && page >= 1 && page <= Math.ceil(currentSchedules.length / ITEMS_PER_PAGE) && page !== currentPage) {
            currentPage = page;
            renderTable();
            renderPagination();
    ...
    $('ul.nav.side-menu a[href="index5.html"]').closest('li').addClass('current-page active');
    $('ul.nav.side-menu a[href="index5.html"]').closest('li.current-page').parents('li').addClass('active');
});
</script>
