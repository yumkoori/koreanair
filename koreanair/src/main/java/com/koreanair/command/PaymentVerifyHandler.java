...
public class PaymentVerifyHandler implements CommandHandler {
    ...
    public String process(HttpServletRequest request, HttpServletResponse response) throws Exception {
        try {
            ...
            System.out.println("[PaymentVerify] CSRF 토큰 검증 시작");
            System.out.println("[PaymentVerify] 요청받은 CSRF 토큰: " + csrfToken);
            System.out.println("[PaymentVerify] 세션ID: " + (session != null ? session.getId() : "null"));
            
            if (session == null) {
                System.err.println("[PaymentVerify] 세션 검증 실패 - 세션이 null");
            ...
            if (!TokenUtil.validateCSRFToken(session, csrfToken)) {
                System.err.println("[PaymentVerify] CSRF 토큰 검증 실패 - 토큰: " + csrfToken + ", 세션ID: " + session.getId());
            ...
            System.out.println("[PaymentVerify] CSRF 토큰 검증 성공 - 세션ID: " + session.getId());
            
            // 2. impUid 파라미터 받기
            String impUid = request.getParameter("imp_uid");

            System.out.println("[PaymentVerify] 결제 검증 요청 - imp_uid: " + impUid);

            // 3. impUid 유효성 검사
            if (impUid == null || impUid.trim().isEmpty()) {
                System.err.println("[PaymentVerify] imp_uid 누락 오류");
            ...
            if (isValid) {
                System.out.println("[PaymentVerify] 결제 검증 성공 - imp_uid: " + impUid);
                ...
                if (updateSuccess) {
                    ...
                    System.out.println("[PaymentVerify] 결제 검증 및 DB 업데이트 성공 - imp_uid: " + impUid + ", merchant_uid: " + merchantUid);
                    ...
                    System.err.println("[PaymentVerify] 결제 검증은 성공했지만 DB 업데이트 실패 - imp_uid: " + impUid + ", merchant_uid: " + merchantUid);
                    
                    return "/views/error.jsp";
                }
            } else {
                // 검증 실패 시
                System.err.println("[PaymentVerify] 결제 검증 실패 - imp_uid: " + impUid);
            ...
            // 입력값 오류
            System.err.println("[PaymentVerify] 입력값 오류: " + e.getMessage());
            ...
            // 시스템 오류
            System.err.println("[PaymentVerify] 시스템 오류: " + e.getMessage());
            e.printStackTrace();
